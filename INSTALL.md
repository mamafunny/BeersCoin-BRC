# Installing Beerscoin Core

### Pre-compiled binaries

The easiest way to install the latest version of the Beerscoin Core software is
by to download the latest precompiled binaries for your platform from the
[release page](https://github.com/BeersCoin/BeersCoin-BRC/releases). Currently,
...
It is possible to build your own copy of Beerscoin Core with the exact, tested,
dependencies, as used for the binary releases, by using the
[depends system](depends/description.md). Please refer to the
[depends README](depends/README.md) for instructions to build Beerscoin using
these dependencies.

### Compiling using system-provided libraries

  The following are developer notes on how to build Beerscoin on your native
  ...
  - [macOS Build Notes](doc/Building-Beerscoin-1.14-for-Mac.md)
...
Run with the `-testnet` option to run with "play beerscoins" on the test network, if you
...
Beerscoin Core is a multithreaded application, and deadlocks or other multithreading bugs

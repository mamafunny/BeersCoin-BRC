...
const char * const BITCOIN_CONF_FILENAME = "beerscoin.conf";
const char * const BITCOIN_PID_FILENAME = "beerscoind.pid";
...
#else
    const char* pszModule = "beerscoin";
#endif
    if (pex)
        return strprintf(
...
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Beerscoin";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/Beerscoin";
#else
    // Unix
    return pathRet / ".beerscoin";
#endif
#endif
}

...
#if defined(NDEBUG)
# error "Beerscoin cannot be compiled without assertions."
#endif
...
const std::string strMessageMagic = "Beerscoin Signed Message:\n";
                              ...
                              bool fOverrideMempoolLimit, const CAmount& nAbsurdFee, std::vector<uint256>& vHashTxnToUncache)
{
...

    {
        ...
        if (fLimitFree && nModifiedFees < GetBeerscoinMinRelayFee(tx, nSize, !fLimitFree))
        {
...
bool CheckTxInputs(const CChainParams& params, const CTransaction& tx, CValidationState& state, const CCoinsViewCache& inputs, int nSpendHeight)
{
        ...
        for (unsigned int i = 0; i < tx.vin.size(); i++)
        {
            ...
            if (coins->IsCoinBase()) {
                // Beerscoin: Switch maturity at depth 145,000
...
void ThreadScriptCheck() {
    RenameThread("beerscoin-scriptch");
                  ...
                  CCoinsViewCache& view, const CChainParams& chainparams, bool fJustCheck)
{
    ...
    // Beerscoin: BIP30 has been active since inception
    ...
    // Beerscoin: BIP16 has been enabled since inception
    ...
    CAmount blockReward = nFees + GetBeerscoinBlockSubsidy(pindex->nHeight, chainparams.GetConsensus(pindex->nHeight), hashPrevBlock);
...
bool ContextualCheckBlockHeader(const CBlockHeader& block, CValidationState& state, const CBlockIndex* pindexPrev, int64_t nAdjustedTime)
{
    ...
    // Beerscoin: Disallow AuxPow blocks before it is activated.
    ...
    // Beerscoin: Version 2 enforcement was never used
...
bool ContextualCheckBlock(const CBlock& block, CValidationState& state, const CBlockIndex* pindexPrev)
{
    ...
    // Beerscoin: We probably want to disable this
    int nLockTimeFlags = 0;

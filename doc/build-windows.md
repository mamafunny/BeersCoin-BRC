...
Below are some notes on how to build Beerscoin Core for Windows.
...
Beerscoin GitHub repository](https://github.com/beerscoinppc/beerscoin/)).
...
Note that for WSL v1 the Beerscoin Core source path MUST be somewhere in the default mount file system, for
example /usr/src/beerscoin, AND not under, for example, /mnt/d/beerscoin. 
...
Ensure that this toolchain can build for `posix` (else you may have compilation errors for Beerscoin's dependencies):
...
way. This will install to `c:\workspace\beerscoin`, for example:

    make install DESTDIR=/mnt/c/workspace/beerscoin

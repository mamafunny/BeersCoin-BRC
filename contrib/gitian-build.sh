...
# BRC TODO Change these urls to beerscoin.com and add a depends subdomain which will do what this does
# Dependencies
...
SIGNER=
VERSION=
url=https://github.com/BeersCoin/BeersCoin-BRC
proc=2
mem=2000
...
Standalone script to perform the gitian build of Beerscoin Core. Perform
...
-u|--url repo       Specify the URL of the repository. Default is https://github.com/BeersCoin/BeersCoin-BRC
...
function move_build_files() {
    find build/out -type f -exec mv '{}' $outputDir/beerscoin-binaries/${VERSION}/ \;
}
...
if [[ $build == true ]]; then
    # Make output folder
    mkdir -p $outputDir/beerscoin-binaries/"$VERSION"

    pushd ./gitian-builder || exit 1

    # Clean beerscoin git directory because of old caching
    if [ -d inputs/beerscoin/ ]; then
        echo "Cleaning Beerscoin directory..."
        rm -rf inputs/beerscoin/
    fi
    ...
    for descriptor in "${DESCRIPTORS[@]}"; do
        ...
        echo ""
        ./bin/gbuild -j "$proc" -m "$mem" --commit beerscoin="$COMMIT" --url beerscoin="$url" ../gitian-descriptors/gitian-"$descriptor".yml  || exit 1
